#-----------------------------------------------------------------------------
# general config
set(CTEST_PROJECT_NAME "SENSEI")
set(DASHBOARD_TYPE $ENV{DASHBOARD_TYPE})
if (NOT DEFINED ALWAYS_UPDATE)
    if (DASHBOARD_TYPE STREQUAL "Continuous")
        set(ALWAYS_UPDATE OFF)
    else()
        set(ALWAYS_UPDATE ON)
    endif()
endif()
if (NOT DEFINED INCREMENTAL_BUILD)
    if (DASHBOARD_TYPE STREQUAL "Continuous")
        set(INCREMENTAL_BUILD ON)
    else()
        set(INCREMENTAL_BUILD OFF)
    endif()
endif()
if (NOT DEFINED DASHBOARD_RELIABLE)
    set(DASHBOARD_TRACK "Experimental")
else()
    if (NOT DEFINED DASHBOARD_TRACK)
        set(DASHBOARD_TRACK ${DASHBOARD_TYPE})
    endif()
endif()
set(CTEST_SOURCE_DIRECTORY "${CTEST_DASHBOARD_ROOT}/${DASHBOARD_TYPE}/sensei-${SENSEI_BACKEND}/sensei-${SENSEI_BRANCH}")
set(CTEST_BINARY_DIRECTORY "${CTEST_DASHBOARD_ROOT}/${DASHBOARD_TYPE}/sensei-${SENSEI_BACKEND}/sensei-build-${SENSEI_BRANCH}")
if (NOT CTEST_TEST_TIMEOUT)
    set(CTEST_TEST_TIMEOUT 120)
endif()
set(CTEST_UPDATE_COMMAND "${CTEST_GIT_COMMAND}")
#-----------------------------------------------------------------------------
# run the tests
if (NOT INCREMENTAL_BUILD)
    ctest_empty_binary_directory(${CTEST_BINARY_DIRECTORY})
    file(WRITE "${CTEST_BINARY_DIRECTORY}/CMakeCache.txt"
        ${INITIAL_CACHE})
endif()
set(nSrcUpdated 0)
set(nDataUpdated 0)
set(nTestUpdated 0)
set(nUpdated 0)
ctest_start("${DASHBOARD_TRACK}")
ctest_update(SOURCE "${CTEST_SOURCE_DIRECTORY}" RETURN_VALUE nSrcUpdated)
math(EXPR nUpdated "${nSrcUpdated}")
if (ALWAYS_UPDATE OR (nUpdated GREATER 0))
    #ctest_read_custom_files("${CTEST_BINARY_DIRECTORY}")
    ctest_configure(RETURN_VALUE terr)
    ctest_submit(PARTS Update Configure RETRY_DELAY 15 RETRY_COUNT 10)
    if (NOT terr EQUAL 0)
         message(FATAL_ERROR "ERROR: ctest configure failed!")
    endif()
    ctest_build(RETURN_VALUE terr)
    ctest_submit(PARTS Build RETRY_DELAY 15 RETRY_COUNT 10)
    if (NOT terr EQUAL 0)
         message(FATAL_ERROR "ERROR: ctest build failed!")
    endif()
    ctest_test(RETURN_VALUE terr)
    ctest_submit(PARTS Test RETRY_DELAY 15 RETRY_COUNT 10)
    if (NOT terr EQUAL 0)
         message(FATAL_ERROR "ERROR: ctest test failed!")
    endif()
endif()
